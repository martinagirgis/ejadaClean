                    ...
                    <tbody>
                        @foreach($complaints as $complaint)                                
                            <tr>
                                ...
                                <th>{{$complaint->employee->name}}</th>
                                @if($complaint->state == 1)
                                <th>انتظار من المشرف</th>
                                <th>
                                    <a class="btn btn-dark col-sm-5 mr-2" href="{{route('managerClean.Complaint.accept',['id'=>$complaint->id])}}">قبول و عمل مهمة</a>
                                    <a class="btn btn-dark col-sm-5" href="{{route('managerClean.Complaint.refused',['id'=>$complaint->id])}}">رفض</a>
                                </th>
...
<script>
    function getTask()
    {
        ...
        @if ($typee == 'index')
        location.href = "/managerClean/task/index/searsh/" + $date;
            @elseif ($typee == 'now')
            location.href = "/managerClean/complaints/now/searsh/" + $date;
